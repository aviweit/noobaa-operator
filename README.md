...
**Note:** ensure bucket "noobaa-bucket" already exists in minio, otherwise you should create it first
...
retrieve the secrets generated by noobaa

```
build/_output/bin/noobaa-operator-local status --show-secrets=true
```

locate `S3 Credentials` section and invoke the below base on these secrets
...
invoke your `aws s3` commands e.g.,
